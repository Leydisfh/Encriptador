...
function encriptando(){
  ...
  if (/^[a-z\s]*$/.test(textoEntrada)) {
    ...
    alert("El texto solo debe contener letras minúsculas sin caracteres especiales ni acentos.");
  }
...
for(let i = 0; i< codigoArray.length; i++ ){
    if(stringDesencriptado.includes(codigoArray[i][1])){
        stringDesencriptado = stringDesencriptado.replaceAll(codigoArray[i][1],codigoArray[i] [0] )
    }
}
